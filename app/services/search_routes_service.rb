  ...
  public
    ...
    def self.search(route_type, start_city_id, finish_city_id)
      ...
      if @error_messages.empty?
        ...
        return Route.where(route_type: route_type).joins(:route_points).where("((#{check_start_city} OR #{check_finish_city}) )").group("name").group("route_points.point_type").group_by{ |e| e }.select { |k, v| v.size > 1 }.map(&:first)
      else
